...
# This script uses the reddit data to calculate trends in how many upvotes do posts get over time. These trends can be used to see if there are any indications
...
ggplot(log.melted.TimeSeriesUpScore, aes(Date, value, color = variable)) + stat_summary(fun.data = mean_cl_normal) + geom_smooth(method = "lm", formula = y ~ x + 
    x^2 + x^3) + ylab = "Upvote Scores"
theme_classic()
