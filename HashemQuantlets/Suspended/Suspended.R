...
# This script runs analysis on retrived users (most active of authors on the subreddits we are interested in) and calculates the proportion of suspended users
# on each of the subreddits. It also caluclautes the average account age for every post. These measures can be ussed to indicate poor behavior by users and the
# posibility of bots.
...
df = rbind(Conservative, Democrats, HillaryClinton, HillaryMeltdown, Liberal, Libertarian, NeutralPolitics, PoliticalDiscussion, Politics, Resist, SandersForPresident, 
    The_Donald)
...
re = data.frame(Subreddit = as.character(results[, 1]), `Suspended Per 1000` = as.numeric(results[, 2]), `Deleted Per 1000` = as.numeric(results[, 3]), `Average Account Age` = as.numeric(results[, 
    4]))
re[, 2:4] = round(as.numeric(results[, 2:4]), 0)

# Create graphs for suspended users per 1000
suspendedPer.Graph = ggplot(re, aes(reorder(Subreddit, Suspended.Per.1000), Suspended.Per.1000)) + geom_bar(stat = "identity") + xlab("Subreddit") + ylab("Suspended Users per 1000") + 
